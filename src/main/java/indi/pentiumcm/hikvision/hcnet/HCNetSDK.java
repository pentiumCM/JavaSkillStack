...
public interface HCNetSDK extends Library {
    /**
     * 判断操作系统类型，根据不同操作系统加载对应版本的海康sdk库（WIN,LINUX）
     *
     * @return
     */
...
//        	return (HCNetSDK)Native.loadLibrary("\\lib\\HCNetSDK", HCNetSDK.class);
            return (HCNetSDK) Native.loadLibrary("F:\\develop_code\\java\\JavaSkillStack\\src\\lib\\win64\\HCNetSDK",
                    HCNetSDK.class);

        } else {
            // return (HCNetSDK)Native.loadLibrary("/home/MakeAll/libhcnetsdk.so", HCNetSDK.class);
            return (HCNetSDK) Native.loadLibrary("HCNetSDK", HCNetSDK.class);
        }
    }

    Pointer NET_DVR_InitG711Decoder();

    Boolean NET_DVR_DecodeG711Frame(Pointer frame, NET_DVR_AUDIODEC_PROCESS_PARAM s);

    Boolean NET_DVR_ReleaseG711Decoder(Pointer frame);
    ...
    public static final int COMM_VCA_DBD_ALARM = 0x4550;// 驾驶行为报警信息
    ...
    public static final int NET_PLAYM4_ORDER_ERROR = 502;// The order of the function to be called
    // is error.
    ...
    public static final int NET_PLAYM4_CREATE_OFFSCREEN_ERROR = 510;// failed when creating
    // off-screen surface.

    public static final int NET_PLAYM4_BUF_OVER = 511; // buffer is overflow

    public static final int NET_PLAYM4_CREATE_SOUND_ERROR = 512; // failed when creating audio
    // device.

    public static final int NET_PLAYM4_SET_VOLUME_ERROR = 513;// Set volume failed

    public static final int NET_PLAYM4_SUPPORT_FILE_ONLY = 514;// The function only support play
    // file.

    public static final int NET_PLAYM4_SUPPORT_STREAM_ONLY = 515;// The function only support play
    // stream.
    ...
    public static final int NET_PLAYM4_VERSION_INCORRECT = 518; // The version of decoder and
    // encoder is not adapted.
    ...
    public static final int NET_PLAYM4_INIT_TIMER_ERROR = 521; // Initialize multimedia clock
    // failed.
    ...
    public static final int NET_PLAYM4_OPEN_FILE_ERROR_MULTI = 524; // openfile error, streamtype
    // is multi

    public static final int NET_PLAYM4_OPEN_FILE_ERROR_VIDEO = 525; // openfile error, streamtype
    // is video

    public static final int NET_PLAYM4_JPEG_COMPRESS_ERROR = 526; // JPEG compress error

    public static final int NET_PLAYM4_EXTRACT_NOT_SUPPORT = 527; // Don't support the version of
    // this file.
     ...
     **************************************************/
    NativeLong NET_DVR_PlayBackByTime(Long lUserID, NativeLong lChannel, NET_DVR_TIME lpStartTime,
                                      NET_DVR_TIME lpStopTime, String sSavedFileName);

    boolean NET_DVR_GetDVRWorkState_V30(Long lUserID, NET_DVR_WORKSTATE_V30 lpWorkState);

    // JPEG抓图到内存
    boolean NET_DVR_CaptureJPEGPicture_NEW(Long lUserID, NativeLong lChannel, NET_DVR_JPEGPARA lpJpegPara,
                                           ByteBuffer jpegBuffer, int dwPicSize, IntByReference lpSizeReturned);
    ...
    // public static final int COMM_VCA_DBD_ALARM = 0x4550; // 驾驶行为报警信息主动上传

    // public static final int NET_VCA_RULE_ALARM = 0x1202; // 驾驶行为报警信息主动上传
    // public static final int COMM_ALARM_RULE = 0x1202;//行为分析信息上传
    ...
    /*
     * public static interface FMSGCallBack_V31 extends StdCallCallback { public boolean invoke(int lCommand,
     * NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen, Pointer pUser); }
     */
    ...
    public static class NET_VCA_APPEND_INFO extends Structure {
    ...
    public static class NET_DVR_HANDLEEXCEPTION_V30 extends Structure {
        public int dwHandleType; /* 处理方式,处理方式的"或"结果 *//* 0x00: 无响应 *//* 0x01: 监视器上警告 *//*
         * 0x02: 声音警告
         *//*
         * 0x04: 上传中心
         */ /*
...
//        public byte byIvmsChannel;  // Ivms 通
//    }
//
//    /**
//     * 驾驶行为，报警位置信息消息体
//     */
//    public static class NET_DVR_DBD_POSITION_INFO extends Structure
//    {
//        public int bValid;                   /* 定位信息是否有效 */
//        public int dwLongitude;              /* 经度 = 实际度*3600*100+实际分*60*100+实际秒*100 */
//        public int dwLatitude;               /* 纬度 = 实际度*3600*100+实际分*60*100+实际秒*100 */
//        public int dwVehicleSpeed;           /* 速度 = 实际速度*1000*100 */
//        public int dwVehicleDirection;       /* 速度方向角 = 实际方向*100 */
//        public int dwAltitude;                  /* 海拔高度(厘米) */
//    }
//
//    /**
//     * 驾驶行为，报警图片信息
//     */
//    public static class NET_DVR_DBD_PICTURE_INFO extends Structure
//    {
//        public int dwPicType;                /* 报警图片类型 */
//        public Pointer pPicBuf;                  /*图片指针*/
//        public int dwPicLen;                 /* 报警图片长度，为0时表示没有图片 */
//        public int dwTime;                   /* 抓图时间 */
//        public Pointer pVideoBuf;                /*视频指针, xxx.mp4*/
//        public int dwVideoLen;               /* 抓图视频的长度，0表示没有报警视频 20180518 */
//        public byte[] byRes=new byte[12];                /* 预留 */
//    }
//
//    /**
//     * 驾驶行为，告警状态信息
//     */
//    public static class NET_DVR_DBD_ALARM_STATE extends Structure{
//        public byte bySmoke;            /* 吸烟 */
//        public byte byPhone;             /* 打电话 */
//        public byte byTiredDriving;       /* 疲劳驾驶 */
//        public byte byNoVisualFront;      /* 没有目视前方 */
//        public byte byNoHead;             /* 离岗 */
//        public byte byWithoutBelt;        /* 不系安全带 */
//        public byte byPickingUpThing;     /* 捡拾物品 */
//        public byte byYawn;               /* 打哈欠 */
//        public byte byEatOrDrink;         /* 吃喝 */
//        public byte byChatting;          /* 聊天 */
//        public byte byTampering;          /* 遮挡IPC */
//        public byte byWithoutUniform;     /* 未穿工作服 */
//        public byte byDriverCmpFail; /* 身份识别异常*/
//        public byte byDriverChange; /* 驾驶员更换*/
//        public byte byDriveLongTime; /*超时驾驶*/
//        public byte byInfraredBlockingSunglasses;   /* 红外阻断墨镜 */
//        public byte byOutOfWheel;    /* 0-保留，1-单手脱离方向盘，2-双手脱离方向盘 */
...
//    }
    ...
    /*
     * public static class NET_DVR_IPADDR extends Structure { public byte[] sIpV4=new byte[128]; public byte[] byRes =
     * new byte[128]; }
     */
    ...
    public static class NET_DVR_COMPRESSION_INFO_V30 extends Structure {
        ...
        // 9-192K 10-224K 11-256K 12-320K 13-384K 14-448K 15-512K
        // 16-640K 17-768K 18-896K 19-1024K 20-1280K 21-1536K 22-1792K
        // 23-2048最高位(31位)置成1表示是自定义码流, 0-30位表示码流值。

        public int dwVideoFrameRate; // 帧率 0-全部; 1-1/16; 2-1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6;
        // 9-8; 10-10; 11-12; 12-16; 13-20; V2.0版本中新加14-15; 15-18;
        // 16-22;
    ...
    public static class NET_DVR_COMPRESSION_INFO extends Structure {// 码流压缩参数(子结构)
        ...
        public int dwVideoBitrate; // 视频码率 0-保留 1-16K(保留) 2-32K 3-48k 4-64K 5-80K 6-96K 7-128K
        // 8-160k 9-192K 10-224K 11-256K 12-320K
    ...
    public static class NET_DVR_COMPRESSION_INFO_EX extends Structure {// 码流压缩参数(子结构)(扩展) 增加I帧间隔
        ...
        public int dwVideoFrameRate; // 帧率 0-全部; 1-1/16; 2-1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6;
        // 9-8; 10-10; 11-12; 12-16; 13-20, //V2.0增加14-15, 15-18,
        // 16-22;
    ...
    public static class NET_DVR_ALARMINFO_V30 extends Structure {// 上传报警信息(9000扩展)
         ...
         * 触发的录像通道，为1表示对应录像, dwAlarmRelateChannel[0]
         * 对应第1个通道
         */
    ...
    public static class NET_DVR_DBD_ALRAM_INFO extends Structure {
        public int dwSize; /* 报警上传结构体大小 4 */
        public byte byChannel; /* 通道号 1 */
        public byte byLevel; /* 报警等级,1为中度疲劳报警，2为重度疲劳，其他报警默认为1 2019.8.29 */
        public byte[] byRes1 = new byte[2]; /* 保留字节 3 */
        public NET_VCA_DEV_INFO struIpcInfo;
        public NET_DVR_DBD_POSITION_INFO struPosInfo; /* 报警位置信息 24 */
        public NET_DVR_DBD_PICTURE_INFO struPicInfo; /* 报警图片信息 */
        public NET_DVR_DBD_ALARM_STATE struAlarmState; /* 报警状态信息 24 */
        public byte[] byDriverName = new byte[20]; /* 扩展为驾驶员姓名 2019.8.29 */

        @Override
        public String toString() {
            return "NET_DVR_DBD_ALRAM_INFO{" + "dwSize=" + dwSize + ", byChannel=" + byChannel + ", byLevel=" + byLevel
                    + ", byRes1=" + Arrays.toString(byRes1) + ", struIpcInfo=" + struIpcInfo + ", struPosInfo="
                    + struPosInfo + ", struPicInfo=" + struPicInfo + ", struAlarmState=" + struAlarmState
                    + ", byDriverName=" + Arrays.toString(byDriverName) + '}';
        }
    }
    ...
    public static class NET_DVR_AUXOUTCFG extends Structure {
         ...
         * 辅助输出切换时间: 0-不切换,1-5s,2-10s,3-20s,4-30s,5-60s,6
         * -120s,7-300s
         */
    ...
    public static class NET_DVR_FILECOND extends Structure // 录象文件查找条件结构
    {
        ...
        public int dwFileType;// 录象文件类型0xff－全部，0－定时录像,1-移动侦测 ，2－报警触发，3-报警|移动侦测 4-报警&移动侦测 5-命令触发
        // 6-手动录像
    ...
    /*
     * public static interface FVoiceDataCallBack_V30 extends Callback { public void invoke(NativeLong lVoiceComHandle,
     * String pRecvDataBuffer, int dwBufSize, byte byAudioFlag, Pointer pUser); }
     */
    ...
    Long NET_DVR_Login_V30(String sDVRIP, short wDVRPort, String sUserName, String sPassword,
    ...
    boolean NET_DVR_Logout_V30(Long lUserID);
    ...
    boolean NET_DVR_STDXMLConfig(long lUserID, NET_DVR_XML_CONFIG_INPUT lpInputParam,
    ...
    NativeLong NET_DVR_GetFileByTime(Long lUserID, NativeLong lChannel, NET_DVR_TIME lpStartTime,
                                     NET_DVR_TIME lpStopTime, String sSavedFileName);
    ...
    // 升级
    Long NET_DVR_Upgrade(Long lUserID, String sFileName);

    int NET_DVR_GetUpgradeState(Long lUpgradeHandle);

    int NET_DVR_GetUpgradeProgress(Long lUpgradeHandle);

    boolean NET_DVR_CloseUpgradeHandle(Long lUpgradeHandle);
     ...
     * NativeLong NET_DVR_StartVoiceCom_V30(Long lUserID, int dwVoiceChan, boolean bNeedCBNoEncData,
     * FVoiceDataCallBack_V30 fVoiceDataCallBack, Pointer pUser); boolean NET_DVR_SetVoiceComClientVolume(NativeLong
     * lVoiceComHandle, short wVolume);
     */
    ...
    // 参数配置 begin
    boolean NET_DVR_GetDVRConfig(Long lUserID, int dwCommand, NativeLong lChannel, Pointer lpOutBuffer,
    ...
    public static class NET_DVR_AUDIOENC_PROCESS_PARAM extends Structure {
        ...
        public int out_frame_size;// 编码后一帧音频数据的大小(单位为字节)：如果是G711编码，输入数据的大小为320字节；如果是G722编码，输入数据的大小为1280字节
    ...
    public static class NET_DVR_AUDIODEC_INFO extends Structure {
        ...
        public int aacdec_profile; // 编码用的框架（保留）
        public byte[] reserved = new byte[16];
        ; // (保留，置为0);
    }
    ...
    public static class NET_DVR_AUDIODEC_PROCESS_PARAM extends Structure {
        public Pointer in_buf;// 输入数据缓冲区，存放编码前PCM原始音频数据
        public Pointer out_buf;// 输出数据缓冲区，存放编码后音频数据
        public int in_data_size; // 输入缓冲区大小，in_buf存储的数据字节数
        public int proc_data_size; // in_buf缓冲区已被解码的数据大小（单位为字节）
        public int out_frame_size;// 编码后一帧音频数据的大小(单位为字节)：如果是G711编码，输入数据的大小为320字节；如果是G722编码，输入数据的大小为1280字节
        public NET_DVR_AUDIODEC_INFO dec_info;
        public byte g726enc_reset; // G726重置开关(保留)
        public byte g711_type; // G711编码类型：0- U law，1- A law
        public byte[] reserved = new byte[16];
    }
    ...
    /*
     * public static class BYTE_ARRAY extends Structure { public byte[] byValue; public BYTE_ARRAY(int iLen) { byValue =
     * new byte[iLen]; }
     * @Override protected List<String> getFieldOrder() { return Arrays.asList("byValue"); } }
     */
     ...
     * 转发语音数据
     *
     * @param lVoiceComHandle NET_DVR_StartVoiceCom_MR或NET_DVR_StartVoiceCom_MR_V30的返回值
     * @param pOutBuffer      存放语音数据的缓冲区
     * @param dwBufSize       语音数据大小。当前是G722音频编码类型时，每次发送的数据为80字节；当前是G711音频编码类型时，每次发送的数据为160字节。
     * @return
     */
    boolean NET_DVR_VoiceComSendData(int lVoiceComHandle, Pointer pOutBuffer, int dwBufSize);
    ...
    public static interface FVoiceDataCallBack_MR_V30 extends Callback {
        public void invoke(int lVoiceComHandle, Pointer pRecvDataBuffer, int dwBufSize, byte byAudioFlag, String pUser);
    }
